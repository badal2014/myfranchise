...
function the100_widgets_init() {
	...
	register_sidebar( array(
		'name'          => esc_html__( 'Top Bar', 'the100' ),
		'id'            => 'the100-top-bar',
		'description'   => esc_html__( 'Add widgets here.', 'the100' ),
		'before_widget' => '<aside id="%1$s" class="widget %2$s">',
		'after_widget'  => '</aside>',
	...
	register_sidebar( array(
		'name'          => esc_html__( 'Home Categories', 'the100' ),
		'id'            => 'the100-home-categories',
		'description'   => esc_html__( 'Add widgets here.', 'the100' ),
		'before_widget' => '<aside id="%1$s" class="widget %2$s">',
		'after_widget'  => '</aside>',
		'before_title'  => '<h2 class="widget-title">',
	...
	register_sidebar( array(
		'name'          => esc_html__( 'Email Subscribe', 'the100' ),
		'id'            => 'the100-email-subscribe',
		'description'   => esc_html__( 'Add widgets here.', 'the100' ),
		'before_widget' => '<aside id="%1$s" class="widget %2$s">',
		'after_widget'  => '</aside>',
		'before_title'  => '<h2 class="widget-title">',
...
function clt_fun() {
     $args = array( 'post_type' => 'clients_logo', 'posts_per_page'=> -1 );
          $loop = new WP_Query( $args );
        ?>  
        <div class="clients-logos-top">
		<ul> 
    	<?php
...
add_shortcode( 'Clients_Logo', 'clt_fun' );

function fc_fun() {
     $args = array( 'post_type' => 'franchise_company', 'posts_per_page'=> -1 );
          $loop = new WP_Query( $args );
        ?>  
        ...
        while ( $loop->have_posts() ) : $loop->the_post();  
   		 ?>        
			<li><a href="<?php the_permalink(); ?>"><?php the_post_thumbnail(); ?></a><p>Investment:<br /><?php the_field('price'); ?></p></li>	
 ...
 <?php
  ...
  add_shortcode( 'Franchise_Company', 'fc_fun' );
  ...
  function post_fun() {
     $args = array( 'post_type' => 'post', 'posts_per_page'=> 4 );
    	...
    	<?php
        while ( $loop->have_posts() ) : $loop->the_post();  
   		 ?>  <div class="col-md-3 post-home"> <?php the_post_thumbnail(); ?> 
...
function query_post_type($query) {
  if( is_category() ) {
    ...
    if($post_type)
        ...
        $post_type = array('nav_menu_item', 'post', 'clients_logo'); // don't forget nav_menu_item to allow menus to work!
